...
AST-instrumented test coverage: true per-line / per-branch / per-condition
coverage. The core is repo-agnostic; each repo plugs in via a `RepoAdapter`
(see [Architecture](#architecture)). Ported here from the ModulesOnRails
monorepo, whose adapter and `tiniest` framework references remain below.

## Usage in this repo (Fusion)

```sh
lune run coverage.luau                     # instrument src/, run the TestEZ suite, report
lune run coverage.luau --per-file --recommend
```

Fusion is a single package on the **Roblox pipeline** — every spec reaches for
`game`, so Lune can't host the suite. The adapter
(`lune/adapters/fusion.luau`) instruments `src/` into `.coverage/Fusion/build`,
mounts it as `ReplicatedStorage.Fusion` alongside the real `test/` tree, and
runs the TestEZ bootstrap the way `test/init.server.luau` does (fake scheduler,
`SpecExternal`). Needs `lune`, `rojo`, `cargo`, and either a local Studio
(`run-in-roblox`, the default) or Open Cloud credentials
(`--roblox-runner=opencloud`, see `tools/ci/README.md`).

Roblox-pipeline limitations apply: `--mutate` is skipped (one Studio launch per
mutant), and `--per-test` is a no-op (TestEZ has no plugin hook to snapshot
counters between tests).

## Usage (ModulesOnRails)
...
tools/coverage/
  ...
  lune/
    ...
    adapters/fusion.luau      this repo's adapter
    adapters/modules_on_rails.luau   the upstream repo's adapter
